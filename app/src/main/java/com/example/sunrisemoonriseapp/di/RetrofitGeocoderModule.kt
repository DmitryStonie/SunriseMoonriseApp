...
object RetrofitGeocoderModule {
    ...
    @Singleton
    @Named("geocoder")
    fun provideSunRetrofit(@Named("geocoderApiUrl") BASE_URL : String): Retrofit = Retrofit.Builder()
    ...
    @Provides
    @Singleton
    fun provideSunriseService(@Named("geocoder") retrofit: Retrofit): GeocoderService = retrofit.create(
        GeocoderService::class.java)

    @Provides
    @Singleton
    fun provideMainRemoteData(geocoderService: GeocoderService): GeocoderRemoteData =
